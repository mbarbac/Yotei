...
public class LambdaParser
{
    ...
    public static LambdaParser Parse(Delegate expression, params object?[] concretes)
    {
        ...
        for (int i = 0; i < pars.Length; i++)
        {
            ...
            if (IsDynamic(par)) // Capturing dynamic arguments...
            {
                ...
                var dyn = new LambdaNodeArgument(parser, name);
        ...
        lock (SyncRoot)
        {
            try
            {
                ...
                parser.Result = parser.LastNode ?? parser.ToLambdaNode(obj);
                return parser;
            }
            finally
            {
    ...
    /// until and exclusive lock can be captured on this object, then sets this current parser
    /// as the global <see cref="Current"/> one, and then actually executes the given dynamic
    /// lambda expression.
    /// </summary>
    ...
    /// <summary>
    /// The current parser under the synchronization root, or null if any.
    /// </summary>
    internal static LambdaParser? Current { get; set; } = null;
    ...
    /// <summary>
    /// Keeps track of the dynamic lambda nodes that shall be used as surrogates for tracked
    /// objects.
    /// </summary>
    ...
    {
        if (value != null &&
            Surrogates.TryGetValue(value, out var node)) return node;

        return value switch
        {
            LambdaNode item => item,
            LambdaMetaNode meta => meta.LambdaNode,
            DynamicMetaObject meta => ToLambdaNode(meta.Value),
