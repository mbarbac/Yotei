...
{
    /// <summary>
    /// Determines if execution of tests shall fail as soon as the first error is detected,
    /// or rather that error is just reported and the remaining tests get executed.
    /// </summary>
        ...
        foreach (var assemblyHolder in assemblyHolders)
        {
            ...
            foreach (var typeHolder in assemblyHolder.TypeHolders)
            {
                ...
                foreach (var methodHolder in typeHolder.MethodHolders)
                {
                    ...
                    var key = ConsoleEx.ReadKey(false, TimeSpan.Zero);
