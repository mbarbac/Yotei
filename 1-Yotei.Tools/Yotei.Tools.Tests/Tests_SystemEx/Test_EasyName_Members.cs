...
// ========================================================
//[Enforced]
public static class Test_EasyNames_Members
{
    const string NAMESPACE = "Yotei.Tools.Tests";
    const string CLASSNAME = nameof(Test_EasyNames_Members);
    ...
    public static void Test_Method_Unbounded()
    {
        ...
        options = options with { UseMemberType = EasyNameOptions.Default };
        name = item.EasyName(options); Assert.Equal("T M", name);

        options = options with { UseMemberHost = EasyNameOptions.Default };
        name = item.EasyName(options); Assert.Equal("T TA<K, T>.M", name);

        options = options with { UseMemberGenericArguments = EasyNameOptions.Default };
        name = item.EasyName(options); Assert.Equal("T TA<K, T>.M<S>", name);

        options = options with { UseMemberArguments = true };
        name = item.EasyName(options); Assert.Equal("T TA<K, T>.M<S>(,)", name);

        options = options with { UseMemberArgumentsTypes = EasyNameOptions.Default };
