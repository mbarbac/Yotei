...
public static class Test_EasyName_Members
{
    ...
    public static void Test_Method_Unbounded()
    {
        ...
        var name = item.EasyName(options); Assert.Equal("", name);

        options = options with { UseMemberName = true };
        name = item.EasyName(options); Assert.Equal("M", name);

        options = options with { UseMemberArguments = true };
        name = item.EasyName(options); Assert.Equal("M(,)", name);

        options = options with { UseTypeArguments = true };
        options = options with { UseMemberTypeArguments = true };
        name = item.EasyName(options); Assert.Equal("M<>(,)", name);

        options = options with { UseTypeName = true };
        name = item.EasyName(options); Assert.Equal("M<S>(,)", name);

        options = options with { UseMemberArgumentsTypes = true };
        name = item.EasyName(options); Assert.Equal("M<S>(K, S)", name);

        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("M<S>(K one, S two)", name);

        options = options with { UseMemberType = true };
        name = item.EasyName(options); Assert.Equal("T M<S>(K one, S two)", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("T TA<K, T>.M<S>(K one, S two)", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"T {CLASSNAME}.TA<K, T>.M<S>(K one, S two)", name);
    ...
    public static void Test_Method_Bounded()
    {
        ...
        options = options with { UseTypeName = true };
        name = item.EasyName(options); Assert.Equal("M<Int32>(,)", name);

        options = options with { UseMemberArgumentsTypes = true };
        name = item.EasyName(options); Assert.Equal("M<Int32>(Byte, Int32)", name);

        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("M<Int32>(Byte one, Int32 two)", name);

        options = options with { UseMemberType = true };
        name = item.EasyName(options); Assert.Equal("String M<Int32>(Byte one, Int32 two)", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("String TA<Byte, String>.M<Int32>(Byte one, Int32 two)", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"String {CLASSNAME}.TA<Byte, String>.M<Int32>(Byte one, Int32 two)", name);
    ...
    public static void Test_Implicit_Constructor_Unbounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("TB<T> TB<T>.new()", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"{CLASSNAME}.TB<T> {CLASSNAME}.TB<T>.new()", name);
    ...
    public static void Test_Implicit_Constructor_Bounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("TB<Byte> TB<Byte>.new()", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"{CLASSNAME}.TB<Byte> {CLASSNAME}.TB<Byte>.new()", name);
    ...
    public static void Test_Explicit_Constructor_Unbounded()
    {
        ...
        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("new(K one, T two)", name);

        options = options with { UseMemberType = true };
        name = item.EasyName(options); Assert.Equal("TC<K, T> new(K one, T two)", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("TC<K, T> TC<K, T>.new(K one, T two)", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"{CLASSNAME}.TC<K, T> {CLASSNAME}.TC<K, T>.new(K one, T two)", name);
    ...
    public static void Test_Explicit_Constructor_Bounded()
    {
        ...
        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("new(Byte one, String two)", name);

        options = options with { UseMemberType = true };
        name = item.EasyName(options); Assert.Equal("TC<Byte, String> new(Byte one, String two)", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("TC<Byte, String> TC<Byte, String>.new(Byte one, String two)", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"{CLASSNAME}.TC<Byte, String> {CLASSNAME}.TC<Byte, String>.new(Byte one, String two)", name);
    ...
    public static void Test_Indexer_Unbounded()
    {
        ...
        options = options with { UseMemberArgumentsTypes = true };
        name = item.EasyName(options); Assert.Equal("T this[K, T]", name);

        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("T this[K one, T two]", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("T TD.this[K one, T two]", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"T TD<K, T>.this[K one, T two]", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"T {CLASSNAME}.TD<K, T>.this[K one, T two]", name);
    ...
    public static void Test_Indexer_Bounded()
    {
        ...
        options = options with { UseMemberArgumentsTypes = true };
        name = item.EasyName(options); Assert.Equal("String this[Byte, String]", name);

        options = options with { UseMemberArgumentsNames = true };
        name = item.EasyName(options); Assert.Equal("String this[Byte one, String two]", name);

        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("String TD.this[Byte one, String two]", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"String TD<Byte, String>.this[Byte one, String two]", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"String {CLASSNAME}.TD<Byte, String>.this[Byte one, String two]", name);
    ...
    public static void Test_Custom_Indexer_Unbounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("T TE.this[K one, T two]", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"T TE<K, T>.this[K one, T two]", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"T {CLASSNAME}.TE<K, T>.this[K one, T two]", name);
    ...
    public static void Test_CustomIndexer_Bounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("String TE.this[Byte one, String two]", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"String TE<Byte, String>.this[Byte one, String two]", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"String {CLASSNAME}.TE<Byte, String>.this[Byte one, String two]", name);
    ...
    public static void Test_Property_Unbounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("T TF.P", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"T TF<K, T>.P", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"T {CLASSNAME}.TF<K, T>.P", name);
    ...
    public static void Test_Property_Bounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("String TF.P", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"String TF<Byte, String>.P", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"String {CLASSNAME}.TF<Byte, String>.P", name);
    ...
    public static void Test_Field_Unbounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("T TG.F", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"T TG<K, T>.F", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"T {CLASSNAME}.TG<K, T>.F", name);
    ...
    public static void Test_Field_Bounded()
    {
        ...
        options = options with { UseMemberHost = true };
        name = item.EasyName(options); Assert.Equal("String TG.F", name);

        options = options with { UseTypeArguments = true };
        name = item.EasyName(options); Assert.Equal($"String TG<Byte, String>.F", name);

        options = options with { UseTypeHost = true };
        name = item.EasyName(options); Assert.Equal($"String {CLASSNAME}.TG<Byte, String>.F", name);
