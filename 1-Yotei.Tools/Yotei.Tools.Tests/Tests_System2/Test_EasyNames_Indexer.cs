...
public static class Test_EasyNames_Indexer
{
    ...
    public static void Test_Indexer_Standard()
    {
        ...
        Assert.Equal(
            $"System.Int32 {NAMESPACE}.{CLASSNAME}.TXA.TA.this[System.Byte one, System.String two]",
            name);
    }

    // -----------------------------------------------------

    public class TXB { public class TB { [IndexerName("Value")] public int this[byte one, string two] => default; } }

    //[Enforced]
    [Fact]
    public static void Test_Indexer_ChangedItemName()
    {
        // Default...
        var type = typeof(TXB.TB);
        var item = type.GetProperty("Item")!; Assert.Null(item);
        item = type.GetProperty("Value")!;
        var options = OPTIONS;
        var name = item.EasyName(options); Assert.Equal("this[Byte, String]", name);
        ...
        name = item.EasyName(options);
        Assert.Equal(
            $"System.Int32 {NAMESPACE}.{CLASSNAME}.TXB.TB.this[System.Byte one, System.String two]",
            name);
    }
    /*
     // -----------------------------------------------------
