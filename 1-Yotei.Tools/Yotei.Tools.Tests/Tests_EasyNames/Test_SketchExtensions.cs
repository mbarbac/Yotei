...
{
    static readonly string NAMESPACE = "Yotei.Tools.Tests.EasyNames";
    static readonly string TESTHOST = typeof(Test_SketchExtensions).Name;
    ...
    public static void Test_Enum_Simple()
    {
        ...
        options = FULL;
        name = source.Sketch(options); Assert.Equal($"{NAMESPACE}.{TESTHOST}.MyEnum.One", name);
    }
    ...
    public static void Test_Enum_Flags()
    {
        ...
        name = source.Sketch(options);
        Assert.Equal(
            $"{NAMESPACE}.{TESTHOST}.MyEnum.One | " +
            $"{NAMESPACE}.{TESTHOST}.MyEnum.Two | " +
            $"{NAMESPACE}.{TESTHOST}.MyEnum.Three",
            name);
    }
    ...
    public static void Test_Shape_ToString_Precedence()
    {
        ...
        options = FULL;
        name = source.Sketch(options);
        Assert.Equal($"({NAMESPACE}.{TESTHOST}.AZ) <AY> & James & 50", name);
