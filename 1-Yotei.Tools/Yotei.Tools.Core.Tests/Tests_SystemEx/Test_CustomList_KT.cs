namespace Yotei.Tools.Core.Tests;

// ========================================================
//[Enforced]
public static class Test_CustomList_KT
{
    ...
    internal class Chain : CustomList<string, Element>
    {
        public Chain(bool sensitive)
        {
            OnValidateItem = (item) =>
            {
                ...
                OnValidateKey(GetKey(item));
                return item;
            };
            OnGetKey = (item) => item.Name;
            OnValidateKey = (key) => key.NotNullNotEmpty();
            OnCompareKeys = (source, target) => string.Compare(source, target, !CaseSensitive) == 0;
            OnAcceptDuplicate = (source, target) =>
            {
