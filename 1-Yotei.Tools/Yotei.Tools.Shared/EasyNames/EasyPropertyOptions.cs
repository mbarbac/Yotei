...
public sealed record EasyPropertyOptions
{
    ...
    /// <summary>
    /// If enabled, and if the member is an indexed property, then use the internal CR name
    /// instead of the standard 'this' one.
    /// </summary>
    public bool UseTechName { get; set; }

    /// <summary>
    /// If enabled, and if the member is an indexed property, then use its square parenthesis
    /// even if no parameter options were specified.
    /// </summary>
    public bool UseBrackets { get; set; }

    /// <summary>
    /// If not null, and if the member is an indexed property, then the options to obtain the
    /// display string of its arguments.If null, then they are ignored.
    /// </summary>
    ...
    public override string ToString()
    {
        ...
        if (ParameterOptions != null) Append($"Parameters:{ParameterOptions}");
        sb.Append(" }");
        return sb.ToString();
