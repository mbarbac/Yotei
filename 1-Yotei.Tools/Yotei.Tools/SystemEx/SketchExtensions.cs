...
public static class SketchExtensions
{
    ...
    public static string Sketch<T>(this T? source, SketchOptions options)
    {
        ...
        var typename = type.EasyName(options.EasyOptions);

        // Null values and inherited cases from 'EasyName'...
        string? str = source switch
        {
            Type item => type.EasyName(options.EasyOptions),
            ConstructorInfo item => item.EasyName(options.EasyOptions),
            MethodInfo item => item.EasyName(options.EasyOptions),
            PropertyInfo item => item.EasyName(options.EasyOptions),
            FieldInfo item => item.EasyName(options.EasyOptions),
        ...
        string GetEnumerable(IEnumerable source)
        {
            ...
            var xoptions = options with { EasyOptions = EasyNameOptions.False };
