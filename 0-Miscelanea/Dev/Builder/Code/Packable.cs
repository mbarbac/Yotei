...
// ========================================================
/// <summary>
/// Represents a project file that produces a NuGet package.
/// </summary>
...
public static class PackageExtensions
{
    ...
    public static ImmutableArray<File> Push(this Packable packable, PushMode mode)
    {
        ...
        var source = mode == PushMode.Local ? "local" : Program.NuGetRepoUrl;
        var files = packable.GetPackageFiles(mode);
        var list = new List<File>();

        var regulars = files.Where(x => string.Compare(x.Extension, "nupkg", Program.Comparison) == 0).ToList();
        foreach (var file in regulars)
        {
            var done = PushFile(file);
            if (done) list.Add(file);
        }
        var symbols = files.Where(x => string.Compare(x.Extension, "snupkg", Program.Comparison) == 0).ToList();
