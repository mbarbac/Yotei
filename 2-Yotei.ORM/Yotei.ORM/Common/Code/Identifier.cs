...
public sealed partial class Identifier : IIdentifier
{
    ...
    /// <summary>
    /// Initializes a new instance with the elements from the given range of values.
    /// </summary>
    /// <param name="engine"></param>
    /// <param name="range"></param>
    public Identifier(
    ...
    string? ValidateKey(string? key, bool wrapped)
    {
        ...
        if (!Engine.UseTerminators) return key;

        if (wrapped) return new IdentifierPart(Engine, key).Value;
        else
        {
            if (key[0] == Engine.LeftTerminator && key[1] == Engine.RightTerminator)
                return new IdentifierPart(Engine, key).UnwrappedValue;

            var dots = Engine.UnwrappedIndexes(key, '.');
            if (dots.Count > 0) throw new ArgumentException("Key contains dots.").WithData(key);
            return key;
        }
    }
    ...
    /// </summary>
    static void Reduce(List<IdentifierPart> items)
    {
        while (items.Count > 0)
        {
    ...
    /// <summary>
    /// Gets a list with the parts of the given value.
    /// </summary>
    ...
    /// <param name="part"></param>
    /// <returns></returns>
    public int IndexOf(string? part) => IndexOf(part, false);
    int IndexOf(string? part, bool wrapped)
    {
        part = ValidateKey(part, wrapped);
        return IndexOf(x => Compare(GetKey(x, wrapped), part));
    }
    ...
    /// <param name="part"></param>
    /// <returns></returns>
    public int LastIndexOf(string? part) => LastIndexOf(part, false);
    int LastIndexOf(string? part, bool wrapped)
    {
        part = ValidateKey(part, wrapped);
        return LastIndexOf(x => Compare(GetKey(x, wrapped), part));
    }
    ...
    /// <param name="part"></param>
    /// <returns></returns>
    public List<int> IndexesOf(string? part) => IndexesOf(part, false);
    List<int> IndexesOf(string? part, bool wrapped)
    {
        part = ValidateKey(part, wrapped);
        return IndexesOf(x => Compare(GetKey(x, wrapped), part));
    }
    ...
    int ReplaceInternal(int index, IIdentifierPart item)
    {
        ...
        return index == 0 && item.Value == null // Special case...
            ? 1
            : InsertInternal(index, item, reduce: true);
    }
    ...
    {
        var clone = Clone();
        var num = clone.ReplaceInternal(index, value);
        return num > 0 ? clone : this;
    }
    int ReplaceInternal(int index, string? value)
    {
        ...
        var temp = Items[index];
        if (parts.Count == 1 && IsSameElement(temp, parts[0])) return 0;
    ...
    {
        var clone = Clone();
        var num = clone.AddInternal(value);
        return num > 0 ? clone : this;
    }
    ...
    {
        var clone = Clone();
        var num = clone.InsertInternal(index, item, reduce: true);
        return num > 0 ? clone : this;
    }
    ...
    {
        var clone = Clone();
        var num = clone.InsertInternal(index, value);
        return num > 0 ? clone : this;
    }
    ...
    {
        var clone = Clone();
        var num = clone.RemoveInternal(part);
        return num > 0 ? clone : this;
    }
    int RemoveInternal(string? part)
    {
        var index = IndexOf(part, wrapped: false);
    ...
    {
        var clone = Clone();
        var num = clone.RemoveLastInternal(part);
        return num > 0 ? clone : this;
    }
    int RemoveLastInternal(string? part)
    {
        var index = LastIndexOf(part, wrapped: false);
        return index >= 0 ? RemoveAtInternal(index) : 0;
    }
    ...
    {
        var clone = Clone();
        var num = clone.RemoveAllInternal(part);
        return num > 0 ? clone : this;
    }
