...
public abstract partial class CoreList<K, T> : ICoreList<K, T>
{
    ...
    /// <returns></returns>
    public virtual bool IncludeDuplicated(T source, T duplicate) => true;

    /// <summary>
    /// Invoked to determine if input elements that are themselves collections of elements of the
    /// type this one is built for shall be flattened before include them in this instance (and so
    /// using their own elements), or rather use the original value instead.
    /// </summary>
    public virtual bool FlattenInputs => true;
    ...
    public virtual int Replace(int index, T value)
    {
        ...
        // Equivalent elements need no replacement...
        if (value is not IEnumerable<T> || !FlattenInputs)
        {
        ...
        // Insertion...
        var num = value is IEnumerable<T> items && FlattenInputs
    ...
    {
        if (value is IEnumerable<T> range && FlattenInputs) return AddRange(range);
    ...
    {
        if (value is IEnumerable<T> range && FlattenInputs) return InsertRange(index, range);
