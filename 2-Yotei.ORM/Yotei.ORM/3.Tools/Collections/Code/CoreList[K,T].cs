...
{
    [Cloneable]
    partial class MyItems : CoreListAbstract<K, T>
    {
        readonly CoreList<K, T> Master;
        public MyItems(CoreList<K, T> master) => Master = master;
    ...
    protected CoreList(CoreList<K, T> source) : this(source.GetKey) => AddRange(source);
