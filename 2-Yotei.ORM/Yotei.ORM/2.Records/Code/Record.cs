...
public partial class Record : IRecord
{
    ...
    protected Record(Record source)
    {
        ...
        if (schema is not null) Items.Schema = schema;
    }
    ...
    public virtual bool Equals(IRecord? other)
    {
        ...
        if (thisschema is null && otherschema is not null) return false;
        if (thisschema is not null && !thisschema.Equals(other.Schema)) return false;
    ...
    /// <inheritdoc/>
    public virtual Record? GetChanges(
        IRecord target, bool orphanSources = false, bool orphanTargets = false)
    {
        var comparer = new ObjectComparer();
        return GetChanges(target, comparer, orphanSources, orphanTargets);
    }

    IRecord? IRecord.GetChanges(
        IRecord target, bool orphanSources, bool orphanTargets)
        => GetChanges(target, orphanSources, orphanTargets);
    ...
    IRecord? IRecord.GetChanges(
        IRecord target,
        IEqualityComparer comparer, bool orphanSources, bool orphanTargets)
        => GetChanges(target, comparer, orphanSources, orphanTargets);
