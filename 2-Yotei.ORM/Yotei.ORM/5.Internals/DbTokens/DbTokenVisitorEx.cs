...
partial record class DbTokenVisitor
{
    ...
    public virtual ICommandInfo.IBuilder VisitRange(IEnumerable<IDbToken> range)
    {
        ...
        foreach (var token in range)
        {
            ...
            if (needed && RangeSeparator is not null) builder.Add(RangeSeparator);
            needed = true;
    ...
    /// <br/> Argument tokens are considered translation artifacts with no database representation
    /// so, by default, this method returns an empty instance.
    /// </summary>
    ...
    /// (but only when UseNullString is true), and then returns either "IS NULL" or "IS NOT NULL"
    /// construction. Otherwise, returns the default binary operation representation.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> This method, by default, visits the chain elements and returns a command info that
    /// represents them all, joining the results with the current range separator.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> This method produces, by default, a 'COALESCE(left, righ)' construct.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given builder.
    /// <br/> This method translates NULL values into the engine's 'NullValueLiteral', provided
    /// that using null strings is enabled by 'UseNullString'. If so, the original null value is
    /// removed from the list of captured parameters.
    /// </summary>
    protected virtual ICommandInfo.IBuilder VisitCommandBuilder(ICommandInfo.IBuilder builder)
    {
        if (UseNullString)
        {
            ...
            for (int i = 0; i < pars.Count; i++)
            {
                ...
                var found = false;
                var index = 0;
                while ((index = finder.Find(text, par.Name, index, Comparison)) >= 0)
                {
            ...
            // Recreating...
            text = CommandInfo.Builder.NamesToOrdinals(text, pars, Comparison);
            builder.Clear();
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> This method produces, by default, a 'CAST(target AS type)' construct, where 'type'
    /// is either its text specification, or its easy name.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> If the identifier name is the same as the name of the dynamic argument of its chain
    /// then this method intercepts it into an empty one. It also removes redundant dots from its
    /// head.
    /// </summary>
    protected virtual ICommandInfo.IBuilder VisitToken(DbTokenIdentifier token)
    {
        ...
        var name = token.Identifier.RawValue.NullWhenDynamicName(darg, caseSensitive: true);
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> This method, by default, joins the text representation of its parameters without any
    /// separators, and then adds the result to any previous contents. This process provides a way
    /// to inject arbitrary contents into the returned text element when needed.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> By convention, values carried by literal tokens ARE NOT captured as arguments, and
    /// are just injected into the returned text element.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> If the method name is the same as the name of the dynamic argument of its chain, as
    /// in 'x => ...x(...)...', then it is treated as an invoke operation. This mechanism is used
    /// because 'x => x.Any()' is interpreted as a method, whereas 'x => x.Any.x(...)' is rather
    /// interpreted as an invoke one.
    ...
    /// <summary>
    /// Determines if the given chain is either empty os consist in just one element that is an
    /// asterisk-only one.
    /// </summary>
    ...
    /// <summary>
    /// If the chain's first and unique element is itself an enumerable object (except strings),
    /// the expands tht element and returns the expansion. Otherwise, returns the original chain.
    /// </summary>
    ...
    public string ChainToAlias(DbTokenChain chain)
    {
        ...
        var id = new IdentifierUnit(Engine, builder.Text);
        var name = Engine.UseTerminators ? id.Value : id.RawValue;
    ...
    /// <summary>
    /// Reduces the given token to a string literal using a raw visitor.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to visit the given token.
    /// <br/> This method produces, by default, a 'IF (left) THEN (middle) ELSE (right)' construct.
    /// </summary>
    ...
    /// <br/>- Null values are not be captured if <see cref="UseNullString"/> is enabled, their
    /// null string representation is injected instead.
    /// </summary>
    protected virtual ICommandInfo.IBuilder VisitToken(DbTokenValue token)
    {
        ...
        {
            var temp = new DbTokenCommandInfo(info);
            return Visit(temp);
        }
        ...
        if (CaptureValues)
        {
            ...
            var builder = new CommandInfo.Builder(Engine, "{0}", [par]);
            return builder;
        }
