...
/// <br/>- Alternate syntax: 'x => x.And(...)' 'x => x.And()...'. Also with 'Or'. The second
/// parameterless form is not recommended.
/// <br/>- Literal syntax: 'x => "Expression"' or 'x => "AND Expression"'. Also with 'OR'.
/// </summary>
/// <remarks>
/// Entries with no connector just concatenate their contents with any previos ones. Otherwise,
/// they add the appropriate space separators with the previous contents and their own ones.
/// </remarks>
public static partial class FragmentWhere
{
    ...
    public partial class Entry : Fragment.Entry
    {
        ...
        {
            // Setters translated to comparisons for SQL-alike convenience...
            ...
            if (Body is DbTokenLiteral literal)
            {
                ...
                var heads = Extractor.Head(main, Engine, true, out var found, "AND", "OR");
                if (found)
                {
                    main = heads.Main.NotNullNotEmpty();
                    Body = new DbTokenLiteral(main);
                    Connector = heads.Spec;
                }

                var parts = Extractor.FirstSeparator(main, Engine, false, out found, "!=", "<>");
                if (found)
                {
                    Extractor.RoundedBrackets(ref parts.Left, ref parts.Right);
                    parts.Left = parts.Left.NotNullNotEmpty();
                    parts.Right = parts.Right.NotNullNotEmpty();

                    main = $"({parts.Left} <> {parts.Right})";
                    Body = new DbTokenLiteral(main);
                    return;
                }
                ...
                if (found)
                {
                    ...
                    main = $"({parts.Left} {parts.Spec} {parts.Right})";
                    Body = new DbTokenLiteral(main);
                    return;
                }
                parts = Extractor.FirstSeparator(main, Engine, false, out found, "==", "=");
