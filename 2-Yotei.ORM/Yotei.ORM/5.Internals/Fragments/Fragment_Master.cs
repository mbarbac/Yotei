...
public static partial class Fragment
{
    ...
    public abstract partial class Master : IEnumerable<Entry>
    {
        ...
        /// <returns></returns>
        public virtual Entry Capture(Func<dynamic, object> spec)
        {
        ...
        /// <param name="spec"></param>
        /// <returns></returns>
        public virtual Entry CreateEntry(Func<dynamic, object> spec)
        {
        ...
        public virtual ICommandInfo.IBuilder Visit()
        {
            ...
            return Visit(visitor, EntryVisitor);
        }

        /// <summary>
        /// Obtains the separator to use between the given entry and the previous ones, if any.
        /// If the returned value is null, the it is ignored and no separator is used.
        /// </summary>
        /// <param name="entry"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Visits the entries in this instance, using the given delegate to visit each of them,
        /// and returns a command info object that can be used to build the related clause of
        /// the associated command. If this instance contains no entries, then an empty result
        /// is returned.
        /// </summary>
        /// <param name="visitor"></param>
        /// <param name="entryVisitor"></param>
        /// <returns></returns>
        protected virtual ICommandInfo.IBuilder Visit(
            DbTokenVisitor visitor,
            Func<Entry, DbTokenVisitor, ICommandInfo.IBuilder> entryVisitor)
        {
            ...
            {
                var entry = Items[i];
                var temp = entryVisitor(entry, visitor);

                if (!temp.IsEmpty)
                {
                    if (i > 0 && builder.TextLen > 0)
                    {
                        var separator = EntrySeparator(entry);
                        if (separator is not null) builder.Add(separator);
                    }
                    builder.Add(temp);
                }
            }
