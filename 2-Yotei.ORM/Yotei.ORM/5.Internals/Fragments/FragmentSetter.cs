    ...
    public partial class Entry : Fragment.Entry
    {
        ...
        public Entry(Master master, IDbToken body) : base(master, body)
        {
            ...
            if (Body is DbTokenLiteral literal)
            {
                ...
                var parts = Extractor.FirstSeparator(main, Engine, false, out var found, "=");
                if (found)
                {
                    Extractor.RoundedBrackets(ref parts.Left, ref parts.Right);

                    Target = new DbTokenLiteral(parts.Left.NotNullNotEmpty());
                    Value = new DbTokenLiteral(parts.Right.NotNullNotEmpty());
                    return;
                }
            }
            ...
            if (Body is DbTokenCommandInfo command)
            {
                ...
                if (found)
                {
                    if (ContainsAnyParameter(parts.Left, 0, info.Parameters))
                        throw new ArgumentException(
                            "Target part cannot contain any encoded parameter.").WithData(Body);

                    Extractor.RoundedBrackets(ref parts.Left, ref parts.Right);
                    main = parts.Right.NotNullNotEmpty();
