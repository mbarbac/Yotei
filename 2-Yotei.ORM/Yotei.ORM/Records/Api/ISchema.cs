...
// ========================================================
/// <summary>
/// Represents the immutable collection of metadata that describes the structure and contents
/// of a record. Elements with matching identifiers are considered duplicated, and are only
/// allowed if they strictly are the same instance.
/// </summary>
[Cloneable]
public partial interface ISchema : IEnumerable<TItem>
{
    ...
    /// <summary>
    /// Returns the elements in this collection with the given key. If there is just one single
    /// match, then it is placed in the out argument. Otherwise, it is set to null.
    /// <br/> Comparison is performed from right to left among the non-terminated values of the
    /// parts of the identifiers of each entry against the dot-separated parts of the given key.
    /// Any null, empty, or missed filter is considered as an implicit match.
    /// </summary>
    /// <param name="key"></param>
    /// <returns></returns>
    List<TItem> Match(TKey key, out TItem? unique);
