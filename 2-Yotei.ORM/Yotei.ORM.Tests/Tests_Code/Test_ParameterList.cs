...
public static class Test_ParameterList
{
    ...
    public static void Test_Create_Single()
    {
        ...
        try { _ = new ParameterList(engine, (IParameter)null!); Assert.Fail(); }
        catch (ArgumentNullException) { }

        try { _ = new ParameterList(engine, new Parameter("", null)); Assert.Fail(); }
    ...
    {
        var source = new ParameterList(new FakeEngine(), [x007, new Parameter("Name", "James")]);
        var target = new ParameterList(new FakeEngine(), [x007, new Parameter("NAME", "James")]);
        Assert.True(source.Equals(target));

        target = new ParameterList(new FakeEngine(), [x007, new Parameter("other", "James")]);
