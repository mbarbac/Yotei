...
{
    public static string? ToUnwrappedString(this IIdentifierChain chain) => chain.Count == 0
        ? null
        : string.Join('.', chain.Select(x => x.UnwrappedValue));
    ...
    public static void Test_Create_Empty_Parts()
    {
        ...
        Assert.Null(chain.ToUnwrappedString());
