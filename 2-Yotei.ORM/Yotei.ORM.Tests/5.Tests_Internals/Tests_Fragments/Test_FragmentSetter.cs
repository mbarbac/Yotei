...
public static class Test_FragmentSetter
{
    ...
    //[Enforced]
    [Fact]
    public static void Test_Literal_From_Invoke_Rounded()
    {
        var command = new FakeCommand(new FakeConnection(new FakeEngine()));
        FragmentSetter.Master master;
        FragmentSetter.Entry entry;
    ...
    public static void Test_Literal_Invalid()
    {
        ...
        try { master.Capture(x => "any = other"); Assert.Fail(); } // Not an invoke one...
        catch (ArgumentException) { }

        try { master.Capture(x => x("any")); Assert.Fail(); }
        catch (ArgumentException) { }

        try { master.Capture(x => x("target=")); Assert.Fail(); }
        catch (EmptyException) { }

        try { master.Capture(x => x("=value")); Assert.Fail(); }
        catch (EmptyException) { }
    }
    ...
    //[Enforced]
    [Fact]
    public static void Test_Dynamic_NullValue()
    {
        var command = new FakeCommand(new FakeConnection(new FakeEngine()));
        FragmentSetter.Master master;
        FragmentSetter.Entry entry;
