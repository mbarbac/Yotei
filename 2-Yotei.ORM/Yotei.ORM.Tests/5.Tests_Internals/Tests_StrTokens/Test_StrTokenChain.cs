...
public static class Test_StrTokenChain
{
    ...
    {
        var comparison = StringComparison.OrdinalIgnoreCase;
        var source = new Chain([xone, xtwo, new Literal("three"), xfour]);
        var target = source.Reduce(comparison);
        Chain chain;
        Text text;
        Literal literal;
        ...
        Assert.NotSame(source, target);
        chain = Assert.IsType<Chain>(target);
        Assert.Equal(3, chain.Count);
        text = Assert.IsType<Text>(chain[0]); Assert.Equal("onetwo", text.Payload);
        literal = Assert.IsType<Literal>(chain[1]); Assert.Equal("three", literal.Payload);
        text = Assert.IsType<Text>(chain[2]); Assert.Equal("four", text.Payload);
    }

    //[Enforced]
    [Fact]
    public static void Test_Reduce_Wrapped()
    {
        var wone = new StrTokenWrapped('[', new StrTokenText("xx"), ']');
        var wtwo = new StrTokenWrapped('[', wone, ']');
        var wthree = new StrTokenWrapped('[', wtwo, ']');

        var comparison = StringComparison.OrdinalIgnoreCase;
        var source = new Chain([xone, xtwo, wthree, xfour]);
        var target = source.Reduce(comparison);
        Chain chain;
        Text text;
        ...
        Assert.Equal(3, chain.Count);
        text = Assert.IsType<Text>(chain[0]); Assert.Equal("onetwo", text.Payload);
        wrapped = Assert.IsType<StrTokenWrapped>(chain[1]); Assert.Equal("[xx]", wrapped.ToString());
        text = Assert.IsType<Text>(chain[2]); Assert.Equal("four", text.Payload);
    }
}