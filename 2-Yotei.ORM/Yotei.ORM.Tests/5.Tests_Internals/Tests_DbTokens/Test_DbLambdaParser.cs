...
public static class Test_DbLambdaParser
{
    ...
    public static void Parse_Command()
    {
        ...
        Assert.NotSame(cmd, command.Command);
    ...
    //[Enforced]
    [Fact]
    public static void Parse_Command_No_Clone()
    {
        IDbToken token;
        DbTokenCommand command;
        DbTokenMethod method;
        ...
        token = parser.Parse(x => x(cmd, false));
        command = Assert.IsType<DbTokenCommand>(token);
        Assert.Same(cmd, command.Command);
        Assert.Equal("(SELECT * FROM Employees WHERE Id = #0)", token.ToString());
        Assert.Equal(token.ToString(), cmd.GetCommandInfo().Text.Wrap('(', ')'));

        token = parser.Parse(x => x(cmd, false).As("Any"));
        method = Assert.IsType<DbTokenMethod>(token);
        Assert.Equal("As", method.Name);
        Assert.Single(method.Arguments);
        Assert.Equal("'Any'", method.Arguments[0].ToString());
    ...
    public static void Parse_Invoke_ToLiteral()
    {
        ...
        text = Assert.IsType<DbTokenLiteral>(token);
    ...
    public static void Parse_Invoke_ToLiteral_Chained()
    {
        ...
        Assert.Equal(2, invoke.Arguments.Count);
        Assert.IsType<DbTokenIdentifier>(invoke.Arguments[0]);
        text = Assert.IsType<DbTokenLiteral>(invoke.Arguments[1]); Assert.Equal("any", text.Value);

        // Last invoke is not a 1st-level one, and so it does not escape its argument...
        token = parser.Parse(x => x(x.Alpha)("33"));
        Assert.Equal("x(x.[Alpha])('33')", token.ToString());
        invoke = Assert.IsType<DbTokenInvoke>(token);
        Assert.Single(invoke.Arguments);
        ...
        token = parser.Parse(x => x(x.Alpha)(x("33")));
        Assert.Equal("x(x.[Alpha])(33)", token.ToString());
        invoke = Assert.IsType<DbTokenInvoke>(token);
        Assert.Single(invoke.Arguments);
    ...
    //[Enforced]
    [Fact]
    public static void Parse_Invoke_ToLiteral_Arguments_Concatenated()
    {
        var parser = new DbLambdaParser(new FakeEngine());
        IDbToken token;
        DbTokenInvoke invoke;
        ...
        literal = Assert.IsType<DbTokenLiteral>(invoke.Host); // 1st level escape...
        Assert.Equal("WHERE ID > ", literal.Value);

        // Now '007' is 1st-level escaped..
        token = parser.Parse(x => x.Any(x("WHERE ID > ")(x("007"))));
        Assert.Equal("x.Any(WHERE ID > (007))", token.ToString());
        method = Assert.IsType<DbTokenMethod>(token);
        Assert.Single(method.Arguments);
        invoke = Assert.IsType<DbTokenInvoke>(method.Arguments[0]);
        Assert.Single(invoke.Arguments);
        literal = Assert.IsType<DbTokenLiteral>(invoke.Arguments[0]); // Not 1st level escape...
    ...
    public static void Parse_Method_AsInvoke_ToLiteral()
    {
        ...
        // 1st-level invoke translated to literal...
        token = parser.Parse(x => x("any"));
        Assert.Equal("any", token.ToString());
        text = Assert.IsType<DbTokenLiteral>(token);
        Assert.Equal("any", text.Value);

        // '...x("any")' is not a first-level invoke...
        token = parser.Parse(x => x.Alpha.x("any"));
        ...
        // But we can force the 1st-level escape...
        token = parser.Parse(x => x.Alpha.x(x("any")));
        Assert.Equal("x.[Alpha](any)", token.ToString());
        invoke = Assert.IsType<DbTokenInvoke>(token);
        id = Assert.IsType<DbTokenIdentifier>(invoke.Host);
        Assert.Equal("[Alpha]", id.Value);
    ...
    public static void Parse_Method_ArgumentInvokeString_As_LiteralArgument()
    {
        ...
        Assert.Equal("x.[Alpha].Beta(x.[Delta], NULL, any)", token.ToString());
        item = Assert.IsType<DbTokenMethod>(token);
        Assert.Equal(3, item.Arguments.Count);
        Assert.IsType<DbTokenIdentifier>(item.Arguments[0]);
        Assert.IsType<DbTokenValue>(item.Arguments[1]);
        text = Assert.IsType<DbTokenLiteral>(item.Arguments[2]); Assert.Equal("any", text.Value);
    }
