...
// ========================================================
//[Enforced]
public static partial class Test_CoreListT
{
    ...
    public static void Test_Create_Range()
    {
        ...
        source = new Chain(true, [xone, new Named("ONE")]);
    ...
    public static void Test_Replace_Same()
    {
        ...
        done = chain.Replace(1, new Named("TWO")); // No duplicated because in same slot
        Assert.Equal(0, done);
        Assert.Equal(3, chain.Count);
        Assert.Same(xone, chain[0]);
        Assert.Same(xtwo, chain[1]);
    ...
    public static void Test_Replace_Standard()
    {
        ...
        var done = chain.Replace(1, xfour);
        Assert.Equal(1, done);
        Assert.Equal(3, chain.Count);
        Assert.Same(xone, chain[0]);
    ...
    public static void Test_Replace_Empty()
    {
        ...
        var done = chain.Replace(1, new Chain(false, []));
        Assert.Equal(0, done);
        Assert.Equal(3, chain.Count);
        Assert.Same(xone, chain[0]);
        Assert.Same(xtwo, chain[1]);
    ...
    public static void Test_Replace_Many()
    {
        ...
        var done = chain.Replace(1, new Chain(false, [new Named("Alpha"), new Named("Beta")]));
        Assert.Equal(2, done);
        Assert.Equal(4, chain.Count);
        Assert.Same(xone, chain[0]);
        Assert.Equal("Alpha", ((Named)chain[1]).Name);
        Assert.Equal("Beta", ((Named)chain[2]).Name);
    ...
    public static void Test_RemoveAt()
    {
        ...
        done = chain.RemoveAt(1, out var item);
        Assert.Equal(1, done);
    ...
    public static void Test_RemoveRange()
    {
        ...
        done = chain.RemoveRange(0, 4, out var items);
        Assert.Equal(4, done);
        Assert.Empty(chain);
        ...
        done = chain.RemoveRange(1, 3, out items);
        Assert.Equal(3, done);
        Assert.Single(chain);
        Assert.Same(xone, chain[0]);
        Assert.Equal(3, items.Count);
        ...
        try { items.RemoveRange(4, 0); Assert.Fail(); } catch (ArgumentException) { }
        try { items.RemoveRange(0, 4); Assert.Fail(); }  catch (ArgumentException) { }
    }
    ...
    public static void Test_Remove_Item()
    {
        ...
        chain = new Chain(false) { IsValidDuplicate = (_, _) => true };
        chain.AddRange([xone, xtwo, xthree, xone]);
        done = chain.RemoveLast(new Named("ONE"));
        Assert.Equal(1, done);
        Assert.Equal(3, chain.Count);
        Assert.Same(xone, chain[0]);
        Assert.Same(xtwo, chain[1]);
        ...
        chain = new Chain(false) { IsValidDuplicate = (_, _) => true };
        chain.AddRange([xone, xtwo, xthree, new Named("ONE")]);
        done = chain.RemoveAll(xone, out var items);
        Assert.Equal(2, done);
        Assert.Equal(2, items.Count);
        Assert.Same(xone, items[0]);
        Assert.Equal("ONE", ((Named)items[1]).Name);
        Assert.Equal(2, chain.Count);
        Assert.Same(xtwo, chain[0]);
        Assert.Same(xthree, chain[1]);
    }
    ...
    public static void Test_Remove_Item_Flatten()
    {
        ...
        chain = new Chain(false) { IsValidDuplicate = (_, _) => true };
        chain.AddRange([xone, xtwo, xthree, xone]);
        done = chain.Remove(new Chain(false, [xtwo, xone]));
        Assert.Equal(2, done);
        Assert.Equal(2, chain.Count);
        Assert.Same(xthree, chain[0]);
        Assert.Same(xone, chain[1]);
        ...
        chain = new Chain(false) { IsValidDuplicate = (_, _) => true };
        chain.AddRange([xone, xtwo, xthree, xone]);
        done = chain.RemoveLast(new Chain(false, [xtwo, xone]));
        Assert.Equal(2, done);
        Assert.Equal(2, chain.Count);
        Assert.Same(xone, chain[0]);
        Assert.Same(xthree, chain[1]);
        ...
        chain = new Chain(false) { IsValidDuplicate = (_, _) => true };
        chain.AddRange([xone, xtwo, xthree, xone]);
        done = chain.RemoveAll(new Chain(false, [xtwo, xone]), out var items);
        Assert.Equal(3, done);
        Assert.Equal(3, items.Count);
        Assert.Same(xtwo, items[0]);
        Assert.Same(xone, items[1]);
