...
public static class Test_ParameterList
{
    ...
    //[Enforced]
    [Fact]
    public static void Test_Create_Builder()
    {
        var engine = new FakeEngine();
        var builder = ParameterList.CreateBuilder(engine);
        Assert.NotNull(builder);
        Assert.Empty(builder);
        Assert.IsAssignableFrom<ICoreList<string, IParameter>>(builder);
    }
    ...
    public static void Test_Builder()
    {
        ...
        Assert.Equal(2, items.Count);
        Assert.Same(xJames, builder[0]);
        Assert.Same(xBond, builder[1]);
    }
