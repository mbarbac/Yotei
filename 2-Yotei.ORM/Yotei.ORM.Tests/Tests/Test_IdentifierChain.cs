...
{
    public static string? ToUnwrappedValue(this IIdentifierChain item) => item.Count == 0
        ? null
    ...
    public static void Test_Create_Empty()
    {
        ...
        var items = new IdentifierChain(engine);
        Assert.Empty(items);
    ...
    {
        var engine = new FakeEngine();
        var items = new IdentifierChain(engine, (string?)null);
        Assert.Empty(items);
        Assert.Null(items.Value);

        items = new IdentifierChain(engine, " ");
        Assert.Empty(items);
        Assert.Null(items.Value);
        ...
        items = new IdentifierChain(engine, "one");
        Assert.Single(items);
    ...
    {
        var engine = new FakeEngine();
        var items = new IdentifierChain(engine, Array.Empty<IdentifierPart>());
        Assert.Empty(items);
        Assert.Null(items.Value);
        ...
        items = new IdentifierChain(engine, [one, two, three]);
    ...
    {
        var engine = new FakeEngine();
        var items = new IdentifierChain(engine, "one.two.three");
        Assert.Equal(3, items.Count);
        Assert.Equal("[one].[two].[three]", items.Value);

        items = new IdentifierChain(engine, "one.two.");
        Assert.Equal(3, items.Count);
        ...
        items = new IdentifierChain(engine, ".two.three");
        Assert.Equal(2, items.Count);
        Assert.Equal("[two].[three]", items.Value);

        items = new IdentifierChain(engine, "..three");
        Assert.Single(items);
    ...
    {
        var engine = new FakeEngine();
        var items = new IdentifierChain(engine, " [ one.two ] ");
        Assert.Single(items);
        Assert.Equal("[one.two]", items.Value);
    ...
    public static void Test_Create_No_Terminators()
    {
        ...
        try { _ = new IdentifierChain(engine, "one two"); Assert.Fail(); }
    ...
    public static void Test_Create_With_Same_Terminators()
    {
        ...
        var items = new IdentifierChain(engine, "'one'");
        ...
        items = new IdentifierChain(engine, "'one'..'three'");
        Assert.Equal(3, items.Count);
    ...
    {
        var engine = new FakeEngine();
        var items = new IdentifierChain(engine, "one.two.one.four");
    ...
    public static void Test_Clone()
    {
        ...
        source = new IdentifierChain(engine, "one.two.three");
        target = source.Clone();
        Assert.NotSame(source, target);
        Assert.Equal(source.Count, target.Count);
        Assert.Equal(source.Value, target.Value);

        source = new IdentifierChain(engine, "one.two.");
        target = source.Clone();
        Assert.NotSame(source, target);
        Assert.Equal(source.Count, target.Count);
        Assert.Equal(source.Value, target.Value);

        source = new IdentifierChain(engine, ".two.three");
        target = source.Clone();
        Assert.NotSame(source, target);
        Assert.Equal(source.Count, target.Count);
        Assert.Equal(source.Value, target.Value);
    ...
    {
        var engine = new FakeEngine();
        var source = new IdentifierChain(engine, "one.two.three");
    ...
    public static void Test_RemoveAt()
    {
        ...
        source = new IdentifierChain(engine, "one..three");
        target = source.RemoveAt(0);
        Assert.NotSame(source, target);
    ...
    public static void Test_Equality()
    {
        ...
        source = new IdentifierChain(new FakeEngine(), "one.two");
        target = new IdentifierChain(new FakeEngine(),"[ONE].[TWO]");
        Assert.Equal(source, target);

        source = new IdentifierChain(new FakeEngine(), "one");
        var part = new IdentifierPart(new FakeEngine(), "[ONE]");
        Assert.True(source.Equals(part));
    }
    ...
    {
        var engine = new FakeEngine();
        var source = new IdentifierChain(engine, "one");
    ...
    {
        var engine = new FakeEngine();
        var source = new IdentifierChain(engine, "two.one");
        ...
        Assert.True(source.Match("one"));
        Assert.True(source.Match(".one"));
        Assert.True(source.Match("two.one"));
