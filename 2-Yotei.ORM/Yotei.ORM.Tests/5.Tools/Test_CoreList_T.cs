...
public static partial class Test_CoreList_T
{
    ...
    [Cloneable]
    internal partial class Chain : CoreList<Element>
    {
        ...
        protected override bool CompareItems(Element source, Element item)
        {
            source.ThrowWhenNull();
            item.ThrowWhenNull();
            return string.Compare(source.Name, item.Name, !CaseSensitive) == 0;
        }
    ...
    public static void Test_Remove_Item()
    {
        ...
        var num = source.Remove(new Element("any"));
        Assert.Equal(0, num);

        num = source.Remove(new Element("ONE"));
        Assert.Equal(1, num);
        ...
        num = source.RemoveLast(new Element("ONE"));
        Assert.Equal(1, num);
        Assert.Equal(3, source.Count);
        Assert.Same(xone, source[0]);
        Assert.Same(xtwo, source[1]);
        ...
        source = new Chain(false, [xone, xtwo, xthree, xone]);
        num = source.RemoveAll(new Element("ONE"));
        Assert.Equal(2, num);
        Assert.Equal(2, source.Count);
        Assert.Same(xtwo, source[0]);
        Assert.Same(xthree, source[1]);
