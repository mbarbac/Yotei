...
// ========================================================
//[Enforced]
public static partial class Test_MetadataTag
{
    //[Enforced]
    [Fact]
    public static void Test_Create_Single()
    {
        var item = new MetadataTag(true, "one");
        Assert.Single(item);
    ...
    {
        var item = new MetadataTag(true, ["one", "two"]);
        Assert.Equal(2, item.Count);
        Assert.Equal("one", item.Default);
        Assert.True(item.Contains("one")); Assert.True(item.Contains("ONE"));
        Assert.True(item.Contains("two")); Assert.True(item.Contains("TWO"));

        try { _ = new MetadataTag(true, (IEnumerable<string>)null!); Assert.Fail(); }
        catch (ArgumentNullException) { }

        try { _ = new MetadataTag(true, []); Assert.Fail(); }
        catch (EmptyException) { }
    }
    ...
    {
        var item = new MetadataTag(false, ["one", "ONE"]);
        Assert.Equal(2, item.Count);
        Assert.Equal("one", item.Default);
    ...
    {
        var source = new MetadataTag(true, ["one", "two", "three", "four"]);
        var target = new MetadataTag(true, ["three", "two", "one", "four"]);
    ...
    {
        var item = new MetadataTag(true, ["one", "two", "three"]);
        Assert.Equal("one", item.Default);
    }
    ...
    {
        var source = new MetadataTag(true, ["one", "two", "three"]);
    ...
    {
        var source = new MetadataTag(true, ["one", "two"]);
    ...
    {
        var source = new MetadataTag(true, ["one"]);
    ...
    public static void Test_Remove()
    {
        ...
        source = new MetadataTag(true, ["one"]);
