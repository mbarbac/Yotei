...
public static class Test_IdentifierMultiPart
{
    ...
    {
        var engine = new FakeEngine();
        var items = new THost(engine, (IEnumerable<TItem>)[]);
        Assert.Empty(items);
        Assert.Null(items.Value);

        items = new THost(engine, (IEnumerable<string?>)[]);
        Assert.Empty(items);
        Assert.Null(items.Value);
    ...
    public static void Test_Replace_Item()
    {
        ...
        target = source.Replace(2, new TItem(engine));
    ...
    public static void Test_Replace_Value()
    {
        var engine = new FakeEngine();
        var source = new THost(engine, "one.two.one.three");
        var target = source.Replace(2, "");
        Assert.Equal(4, target.Count);
        Assert.Equal("[one].[two]..[three]", target.Value);

        target = source.Replace(2, "x.y");
        Assert.Equal(5, target.Count);
        Assert.Equal("[one].[two].[x].[y].[three]", target.Value);
    }
    ...
    public static void Test_Remove()
    {
        ...
        target = source.Remove("TWO.THREE"); // Multipart keys are not supported...
        Assert.Same(source, target);

        target = source.RemoveLast("TWO");
        Assert.Equal(3, target.Count);
        Assert.Equal("[one].[two].[three]", target.Value);

        target = source.RemoveAll("TWO");
        Assert.Equal(2, target.Count);
        Assert.Equal("[one].[three]", target.Value);
    }
