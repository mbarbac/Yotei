...
public static class Test_Parameter
{
    ...
    {
        var source = new Parameter("#one", null);
        var target = new Parameter("#one", null);
        Assert.NotSame(source, target);
        Assert.Equal(source, target);

        source = new Parameter("#one", 50);
        target = new Parameter("#ONE", 50);
        Assert.NotSame(source, target);
        Assert.True(source.Equals(target, false));
    }
