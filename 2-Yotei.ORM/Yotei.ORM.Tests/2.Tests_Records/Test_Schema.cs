using Schema = Yotei.ORM.Records.Code.Schema;
using SchemaEntry = Yotei.ORM.Records.Code.SchemaEntry;
...
public static class Test_Schema
{
    ...
    public static void Test_Match()
    {
        ...
        var items = schema.Match("any", out var unique, out var index);
        Assert.Empty(items);
        Assert.Null(unique);
        Assert.Equal(-1, index);

        items = schema.Match("ID", out unique, out index);
        ...
        items = schema.Match("EMPLOYEES.", out unique, out index);
        Assert.Equal(2, items.Count);
        Assert.Equal(0, items[0]);
        ...
        items = schema.Match("LASTname", out unique, out index);
        Assert.Single(items);
        ...
        items = schema.Match("DBO..", out unique, out index);
        Assert.Single(items);
        Assert.Equal(0, items[0]);
        Assert.NotNull(unique);
        ...
        items = schema.Match("Country", out unique, out index);
        Assert.Empty(items);
        Assert.Null(unique);
        Assert.Equal(-1, index);
    }
