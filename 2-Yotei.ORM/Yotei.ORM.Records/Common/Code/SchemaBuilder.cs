...
public sealed partial class SchemaBuilder : CoreList<K, T>
{
    ...
    public SchemaBuilder(IEngine engine)
    {
        ...
        ValidateKey = (key) =>
        {
            ...
            if (key.Value == null) throw new ArgumentException("Identifier value cannot be null.");
            if (key is IIdentifierChain chain)
            {
                if (chain.Count == 0) throw new ArgumentException("Identifier cannot be empty.");
                if (chain[^1].Value == null) throw new ArgumentException("Value of last part of identifier value cannot be null.").WithData(key);
            }
            return key;
        };
