...
public sealed partial class SchemaEntry : ISchemaEntry
{
    ...
    SchemaEntry(SchemaEntry source) => Items = new SchemaEntryBuilder(
        (IEngine)source.Identifier.Engine,
        source);
    ...
    public override string ToString()
    {
        ...
        {
            if (((IEngine)Identifier.Engine).KnownTags.Contains(item.Tag)) continue;
    ...
    public bool Equals(ISchemaEntry? other)
    {
        ...
        if (!IsPrimaryKey.EqualsEx(other.IsPrimaryKey)) return false;
        if (!IsUniqueValued.EqualsEx(other.IsUniqueValued)) return false;
        if (!IsReadOnly.EqualsEx(other.IsReadOnly)) return false;
