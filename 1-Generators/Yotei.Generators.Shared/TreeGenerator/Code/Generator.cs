...
internal abstract class Generator : IGenerator
{
    ...
    bool ValidateNode(SyntaxNode node, CancellationToken token)
    {
        ...
        switch (CaptureLevel)
        {
            ...
            default: throw new UnreachableException($"Invalid capture level: {CaptureLevel}");
        }
    ...
    /// <summary>
    /// Invoked to obtain the a file name for the top namespace, in reverse order.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to obtain the a file name for the tail type, in reverse order.
    /// </summary>
    /// <param name="captured"></param>
    /// <returns></returns>
