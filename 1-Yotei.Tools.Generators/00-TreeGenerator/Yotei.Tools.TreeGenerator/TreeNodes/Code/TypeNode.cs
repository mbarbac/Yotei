...
public partial class TypeNode : ITreeNode
{
    ...
    /// <returns></returns>
    public virtual TypeNode With(
        BaseTypeDeclarationSyntax syntax, IEnumerable<AttributeData> attributes, SemanticModel model)
    {
    ...
    ITreeNode ITreeNode.With(
        ...
        => With((BaseTypeDeclarationSyntax)syntax, attributes, model);
    ...
    /// <param name="cb"></param>
    /// <returns></returns>
    protected virtual bool OnEmitCore(in TreeContext context, CodeBuilder cb) => true;
    ...
    /// <param name="cb"></param>
    /// <returns></returns>
    protected virtual bool OnEmitChilds(in TreeContext context, CodeBuilder cb)
    {
        var r = true;
        var n = false;
        ...
        foreach (var node in ChildProperties)
        {
            ...
            if (!node.Emit(in context, cb)) r = false;
        }
