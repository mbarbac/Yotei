...
internal abstract class TreeGenerator : IIncrementalGenerator
{
    ...
    protected virtual bool Validate(SyntaxNode node, CancellationToken token)
    {
        ...
        if (node is TypeDeclarationSyntax typeSyntax)
        {
            if (TypeAttributes.Length > 0)
            {
                ...
                return atts.Any(x => CompareSyntax(x.Name, TypeAttributes));
            }
            return false;
        }
        ...
        if (node is PropertyDeclarationSyntax propertySyntax)
        {
            if (PropertyAttributes.Length > 0)
            {
                ...
                return atts.Any(x => CompareSyntax(x.Name, PropertyAttributes));
            }
            return false;
        }
        ...
        if (node is FieldDeclarationSyntax fieldSyntax)
        {
            if (FieldAttributes.Length > 0)
            {
                ...
                return atts.Any(x => CompareSyntax(x.Name, FieldAttributes));
            }
            return false;
        }
        ...
        if (node is MethodDeclarationSyntax methodSyntax)
        {
            if (MethodAttributes.Length > 0)
            {
                ...
                return atts.Any(x => CompareSyntax(x.Name, MethodAttributes));
            }
            return false;
        }
    ...
    /// <summary>
    /// Invoked to determine if the given name syntax matches any of the given attribute names.
    /// </summary>
    /// <param name="syntax"></param>
    /// <param name="names"></param>
    /// <returns></returns>
    static bool CompareSyntax(NameSyntax syntax, string[] names)
    {
        ...
        for (int i = 0; i < names.Length; i++)
        {
            ...
            if (orig.CompareTo(name, StringComparison.Ordinal) == 0) return true;
        }
