...
internal class Hierarchy : INode
{
    ...
    public class FileChildrenList : ChildrenList<FileNode>
    {
        ...
        {
            Compare = (x, y) => x.FileName == y.FileName;
            CanInclude = (item, x) => Compare(item, x)
                ? throw new ArgumentException("Duplicated file name.").WithData(item.FileName)
                : true;
        }
        protected override string ItemToString(FileNode item) => item.FileName.Split('.')[0];
    }
