...
internal abstract class TreeGenerator : IIncrementalGenerator
{
    ...
    Candidate Transform(GeneratorSyntaxContext context, CancellationToken token)
    {
        ...
        if (node is TypeDeclarationSyntax typeSyntax)
        {
            ...
            return CreateCandidate(model, typeSyntax, symbol);
        }
        ...
        if (node is PropertyDeclarationSyntax propertySyntax)
        {
            ...
            return CreateCandidate(model, propertySyntax, symbol);
        }
        // Fields...
        if (node is FieldDeclarationSyntax fieldSyntax)
        {
            ...
            foreach (var item in items)
            {
                if (model.GetDeclaredSymbol(item, token) is IFieldSymbol symbol)
                    return CreateCandidate(model, fieldSyntax, symbol);
            }
        ...
        if (node is MethodDeclarationSyntax methodSyntax)
        {
            ...
            return CreateCandidate(model, methodSyntax, symbol);
        }
    ...
    /// <summary>
    /// Creates a candidate of the appropriate type.
    /// </summary>
    /// <param name="model"></param>
    /// <param name="syntax"></param>
    /// <param name="type"></param>
    /// <returns></returns>
    public virtual TypeCandidate CreateCandidate(
        SemanticModel model, TypeDeclarationSyntax syntax, INamedTypeSymbol type)
        => new TypeCandidate(model, syntax, type);
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public virtual PropertyCandidate CreateCandidate(
        SemanticModel model, PropertyDeclarationSyntax syntax, IPropertySymbol type)
        => new PropertyCandidate(model, syntax, type);
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public virtual FieldCandidate CreateCandidate(
        SemanticModel model, FieldDeclarationSyntax syntax, IFieldSymbol type)
        => new FieldCandidate(model, syntax, type);
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public virtual MethodCandidate CreateCandidate(
        SemanticModel model, MethodDeclarationSyntax syntax, IMethodSymbol type)
        => new MethodCandidate(model, syntax, type);
