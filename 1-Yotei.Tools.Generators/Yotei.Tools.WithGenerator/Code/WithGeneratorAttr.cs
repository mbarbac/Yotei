...
internal static class WithGeneratorAttr
{
    ...
    /// <summary>
    /// Tries to obtain the <see cref="Specs"/> value associated with the given symbol.
    /// </summary>
        ...
        foreach (var at in ats)
        {
            if (at is AttributeData data)
            {
                if (data.ConstructorArguments.Length > 0)
                {
                    ...
                    // We shall not use the following because 'null' is the default value and,
                    // if we use it here, that will stop all downstream searches...
                    // if (item.IsNull)...
                }
            }
    ...
    /// <summary>
    /// Tries to obtaine the <see cref="PreventVirtual"/> value associated with the given symbol.
    /// </summary>
    /// <param name="symbol"></param>
    /// <param name="value"></param>
    /// <returns></returns>
