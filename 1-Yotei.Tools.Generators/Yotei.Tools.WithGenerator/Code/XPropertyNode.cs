...
internal class XPropertyNode : PropertyNode
{
    ...
    public XPropertyNode(PropertyCandidate candidate) : base(candidate) { }
    ...
    public override bool Validate(SourceProductionContext context)
    {
        ...
        if (!Symbol.PropertyHasGetter(context)) return false;

        if (!Symbol.ContainingType.IsInterface() &&
            !Symbol.PropertyHasSetter(context)) return false;
