...
internal class TreeGenerator : IIncrementalGenerator
{
    ...
    /// Notes:
    /// It seems that if we capture the 'Compilation' object then we'll loose the incremental nature
    /// of the generator so that the smallest change will trigger a full source code generation.
    /// Therefore, we shall not capture it.
    ...
    /// <br/> The syntax must be among the supported ones.
    /// </summary>
    ...
    /// <summary>
    /// Invoked to capture an element for source code generation purposes. This method can also
    /// return <see langword="null"/> to ignore the syntax node, or an error candidate instance
    /// to report an error conditions at code generation time.
    /// </summary>
    /// <param name="context"></param>
    /// <param name="token"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Determines if the files will all be emitted in the same folder, or rather in a hierarchy
    /// of folder using all the dot-separated parts of their names, except the last one, as the
    /// folders' names.
    /// </summary>
