...
public interface ICoreList<K, T> : ICloneable, IList<T>, IList, ICollection<T>, ICollection
{
    ...
    /// <summary>
    /// Invoked to determine if the given element can be included in this collection. Returns
    /// 'true' if so, or 'false' if the inclusion operation shall be just ignored. In addition,
    /// it can throw an appropriate exception if duplicates are not allowed.
    /// </summary>
    Func<T, bool> CanInclude { get; set; }
