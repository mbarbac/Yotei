...
public static partial class Test_ElementList_T
{
    ...
    //{
    //    var engine = new FakeEngine();
    //    var builder = new Builder(engine) { AllowDuplicates = true };
...
/*
    ...
    {
        var engine = new FakeEngine();
        var chain = new Chain(engine) { AllowDuplicates = true };
        chain = (Chain)chain.AddRange([xone, xone]);
        

        chain = new Chain(engine) { IgnoreCase = true };
        try { chain.AddRange([xone, new Named("ONE")]); Assert.Fail(); } catch (DuplicateException) { }

        chain = new Chain(engine) { IgnoreCase = true, AllowDuplicates = true };
    ...
    public static void Test_Clone()
    {
        ...
        source = new Chain(engine) { AllowDuplicates = true, IgnoreCase = true };
    ...
    {
        var source = new Chain(engine, [xone, xtwo, xthree]) { IgnoreCase = true };
        var target = source.Replace(0, xone);
        Assert.Same(source, target);

        try { source.Replace(1, xone); Assert.Fail(); } catch (DuplicateException) { }

        source = new Chain(engine) { IgnoreCase = true, AllowDuplicates = true };
    ...
    {
        var source = new Chain(engine) { AllowDuplicates = true, IgnoreCase = true };
